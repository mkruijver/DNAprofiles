ki.dist <- function(hyp.1,hyp.2="UN",hyp.true="UN",freqs.ki,freqs.true=freqs.ki){
...
ibs.dist <- function(hyp.true="UN",freqs){
  ...
  lapply(freqs$loci,function(L){  
    ...
    y <- Zki.ibs.joint.dist.at.locus(hyp.1="UN",hyp.2="UN",hyp.true=hyp.true,f.ki=freqs$freqs[[L]],f.true=freqs$freqs[[L]])
...
ki.ibs.joint.dist <- function(hyp.1,hyp.2="UN",hyp.true="UN",freqs.ki,freqs.true=freqs.ki){
  # unconditional ki,ibs joint dist at all loci in f.ki
  lapply(freqs.ki$loci,function(L) Zki.ibs.joint.dist.at.locus(hyp.1=hyp.1,hyp.2=hyp.2,hyp.true=hyp.true,f.ki=freqs.ki$freqs[[L]],f.true=freqs.ki$freqs[[L]])) 
}

cond.ki.dist <- function(x,hyp.1,hyp.2="UN",hyp.true="UN",freqs.ki,freqs.true=freqs.ki){ 
  # ki dist of profile x and some profiles y, related to x by hyp.true
  tmp <- cond.ki.ibs.joint.dist(x,hyp.1,hyp.2=hyp.2,hyp.true=hyp.true,freqs.ki,freqs.true=freqs.ki)
  lapply(tmp, function(y) list(fx=y$fx,x=y$ki))
}

cond.ki.ibs.joint.dist <- function(x,hyp.1,hyp.2="UN",hyp.true="UN",freqs.ki,freqs.true=freqs.ki){ 
  ...
  for (locus.i in 1:(ncol(x)/2)){
    ...
    if ((locus.name %in% names(freqs.ki$freqs))&(locus.name %in% names(freqs.true$freqs))){
      ...
      tmp <- Zcond.ki.ibs.joint.dist.at.locus(a,b,hyp.1=hyp.1,hyp.2=hyp.2,hyp.true=hyp.true,f.ki=f.ki,f.true=f.true)
...
Zcond.ki.ibs.joint.dist.at.locus<-function(a,b,hyp.1,hyp.2="UN",hyp.true="UN",f.ki,f.true=f.ki){
...
Zki.ibs.joint.dist.at.locus <- function(hyp.1,hyp.2="UN",hyp.true="UN",f.ki,f.true=f.ki){
  ...
  for(i in seq_along(G[,1])){
    X[[i]] <- cbind(Zcond.ki.ibs.joint.dist.at.locus(G[i,1],G[i,2],hyp.1=hyp.1,hyp.2=hyp.2,hyp.true=hyp.true,f.ki=f.ki,f.true=f.true),G.fr[i])
  }
